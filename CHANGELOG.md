...
### Added & Enhanced (Engineering Roadmap Implementation)
- **Seamless Raw Data Inference**: `ModelResult.predict(raw_df)` automatically routes raw DataFrames containing un-encoded categorical strings through the fitted preprocessing pipeline.
- **Automated Hyperparameter Tuning (`data.tune()`)**: Added `data.tune()` and `TuneResult` supporting automated parameter search over cross-validation splits.
- **Advanced Classification Visualizations**: Added `res.predict_proba()`, `res.plot_roc_curve()` (with AUC score), and `res.plot_pr_curve()` (with Average Precision score).
- **Warning & Audit Fixes**: Modernized Seaborn boxplot parameters and single-row constant column checks.

## [0.3.0] - 2026-08-09

### Added & Standardized
- **Standardized Machine Learning Engine (`data.fit()`)**: Unified all ML algorithms (`"rf"`, `"gb"`, `"extra_trees"`, `"tree"`, `"linear"`, `"logistic"`, `"ridge"`, `"lasso"`, `"knn"`, `"svc"`, `"svr"`, `"naive_bayes"`).
- **Linear Feature Importance**: `res.plot_importance()` now supports Linear, Logistic, Ridge, and Lasso models by extracting magnitude of coefficients (`coef_`).
- **Memorable Kwarg Aliases**: Supported bidirectional kwarg aliases (`k` / `neighbors` $\rightarrow$ `n_neighbors`, `c` $\rightarrow$ `C`, `alpha` $\leftrightarrow$ `C`, `smoothing` $\rightarrow$ `var_smoothing`).
